//
//  WWE Constants.swift
//  Labs Injections ROS
//
//  Created by Fool on 7/19/16.
//  Copyright © 2016 Fulgent Wake. All rights reserved.
//

import Foundation

let lastPapChoices = ["", "1 year", "2 years", "3 or more years"]
let periodFrequencyChoices = ["", "14", "21", "24", "26", "28", "30", "32", "34"]
let periodLengthChoices = ["", "1", "2", "3", "4", "5", "6", "7", "8"]
let bloodFlowChoices = ["", "light", "moderate", "heavy"]
let birthControlMethods = ["", "OCP", "condoms", "diaphram", "IUD", "depo injection"]
let selfExamFrequencyChoices = ["", "weekly", "monthly", "yearly"]

let zeroToFourChoices = ["", "0", "1", "2", "3", "4"]
let zeroToFiveChoices = zeroToFourChoices + ["5"]
let zeroToSixChoices = zeroToFiveChoices + ["6"]
